...
void encrypt(char a, char b, int matrix[5][5], char *ciphertext, int cipher_index) {
    ...
    if (row1 == row2) {
        // Move diagonally down-right and wrap around
        ciphertext[cipher_index] = matrix[(row1 + 1) % 5][(col1 + 1) % 5];
        ciphertext[cipher_index + 1] = matrix[(row2 + 1) % 5][(col2 + 1) % 5];
    } else if (col1 == col2) {
        // Move diagonally down-left and wrap around
        ciphertext[cipher_index] = matrix[(row1 + 1) % 5][(col1 + 4) % 5];
        ciphertext[cipher_index + 1] = matrix[(row2 + 1) % 5][(col2 + 4) % 5];
    } else {
        // Swap rows and columns with an offset
        ciphertext[cipher_index] = matrix[row1][(col2 + 1) % 5];
        ciphertext[cipher_index + 1] = matrix[row2][(col1 + 1) % 5];
    }
}
...
void decrypt(char a, char b, int matrix[5][5], char *plaintext, int plain_index) {
    ...
    if (row1 == row2) {
        // Move diagonally up-left and wrap around
        plaintext[plain_index] = matrix[(row1 + 4) % 5][(col1 + 4) % 5];
        plaintext[plain_index + 1] = matrix[(row2 + 4) % 5][(col2 + 4) % 5];
    } else if (col1 == col2) {
        // Move diagonally up-right and wrap around
        plaintext[plain_index] = matrix[(row1 + 4) % 5][(col1 + 1) % 5];
        plaintext[plain_index + 1] = matrix[(row2 + 4) % 5][(col2 + 1) % 5];
    } else {
        // Swap rows and columns with an offset
        plaintext[plain_index] = matrix[row1][(col2 + 4) % 5];
        plaintext[plain_index + 1] = matrix[row2][(col1 + 4) % 5];
    }
}
