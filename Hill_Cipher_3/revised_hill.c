...
void encrypt(char *plaintext, const int key[3][3], const int shift_key[], char *ciphertext, int dim)
{
    ...
    for (int i = 0; i < strlen(plaintext); i += dim)
    {
        for (int j = 0; j < dim; j++)
        {
            block[j] = (plaintext[i + j] - 'A' + shift_key[j]) % MOD;
        }
...
void decrypt(char *ciphertext, const int key[3][3], const int shift_key[], char *plaintext, int dim)
{
    ...
    for (int i = 0; i < len; i += dim)
    {
        ...
        multiplyMatrix(inverseKey, block, result, dim);
        for (int j = 0; j < dim; j++)
        {
            plaintext[i + j] = (result[j] - shift_key[j] + MOD) % MOD + 'A';
        }
    }
    plaintext[len] = '\0';
...
int main()
{
    ...
    int shift_key_vector[3];
    for (int i = 0; i < dim; i++)
    {
        shift_key_vector[i] = shift_key[i] - 'A';
    }

    char ciphertext[100] = {0};
    encrypt(plaintext, key_matrix, shift_key_vector, ciphertext, dim);
    printf("The Encrypted Text: %s\n", ciphertext);

    char decrypted[100] = {0};
    decrypt(ciphertext, key_matrix, shift_key_vector, decrypted, dim);
